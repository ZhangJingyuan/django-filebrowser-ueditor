...
# FILEBROWSER IMPORTS
from filebrowser_ueditor.settings import STRICT_PIL, DIRECTORY, EXTENSIONS, SELECT_FORMATS, ADMIN_VERSIONS, ADMIN_THUMBNAIL, MAX_UPLOAD_SIZE,\
...
from filebrowser_ueditor.templatetags.fb_tags import query_helper
from filebrowser_ueditor.base import FileListing, FileObject
from filebrowser_ueditor.decorators import path_exists, file_exists
from filebrowser_ueditor.storage import FileSystemStorageMixin, StorageMixin
from filebrowser_ueditor.utils import convert_filename
from filebrowser_ueditor import signals
...
# This cache contains all *instantiated* filebrowser_ueditor sites
_sites_cache = {}


def get_site_dict(app_name='filebrowser_ueditor'):
    """
...
def get_default_site(app_name='filebrowser_ueditor'):
    """
...
class FileBrowserSite(object):
    ...
    def __init__(self, name=None, app_name='filebrowser_ueditor', storage=default_storage):
        self.name = name
    ...
    def get_urls(self):
        "URLs for a filebrowser_ueditor.site"
        try:
        ...
        # filebrowser_ueditor urls (views)
    ...
    def createdir(self, request):
        "Create Directory"
        from filebrowser_ueditor.forms import CreateDirForm
        query = request.GET
        path = u'%s' % os.path.join(self.directory, query.get('dir', ''))
    ...
    def detail(self, request):
        ...
        """
        from filebrowser_ueditor.forms import ChangeForm
        query = request.GET
        path = u'%s' % os.path.join(self.directory, query.get('dir', ''))
        fileobject = FileObject(os.path.join(path, query.get('filename', '')), site=self)
...
# Default actions
from filebrowser_ueditor.actions import flip_horizontal, flip_vertical, rotate_90_clockwise, rotate_90_counterclockwise, rotate_180
